...
object Encoding {
    ...
    def step[A](inner: Iteratee[Array[Char], A], partialChar: Option[Array[Byte]] = None)(in: Input[Array[Byte]]): Iteratee[Array[Byte], Iteratee[Array[Char], A]] = {
      in match {
        ...
        case El(data) => {
          ...
          val leftOver: Option[Array[Byte]] = if (byteBuffer.limit() > byteBuffer.position()) {
  ...
  /**
   * Converts a stream of byte arrays to a stream of String arrays, taking into account the possibility of split multi
   * byte characters.  Assumes that a String is an Array[Char] separated by a \n

   * @param decoder The decoder to use.  Defaults to a UTF-8 decoder
  */

  def decodeStrings(decoder: CharsetDecoder = Charset.forName("UTF-8").newDecoder()): Enumeratee[Array[Byte], Array[String]] = new Enumeratee[Array[Byte],Array[String]] {
    /**
     * We carry partialChar as state from the stream
     */
    ...
    def applyOn[A](inner: Iteratee[Array[String], A]) = Cont(step(inner))
  }
}